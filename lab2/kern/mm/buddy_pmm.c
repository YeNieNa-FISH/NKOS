...
struct buddy {
    size_t size;
    uintptr_t *longest;// 数组，用于存储每个节点的最大连续空间
    size_t longest_num_page;// longest数组的大小
    size_t total_num_page;// 总共有多少页
    size_t free_size;// 还有多少空闲页
...
static size_t GetSizePof2(size_t size) {
    size |= size >> 1;
    size |= size >> 2;
    size |= size >> 4;
    size |= size >> 8;
...
static void buddy_init_memmap(struct Page *base, size_t pagenum) {
    ...
    size_t v_size = GetSizePof2(pagenum);
